...
|---|---|---|---|
| 2026-09-23 | Budget ceiling moved from `spend_ledger.jsonl` to Postgres, and a paid run *reserves* its worst case before calling Claude | A per-instance file would start empty on every serverless cold start, so the guard would pass every time and the $5 ceiling would silently stop existing. Reading a total and then deciding is also a race two runs can both win | Keeping the file and relying on one machine; a `threading.Lock` (means nothing across instances) |
| 2026-09-23 | Rerank pool capped at 5,000 Voyage tokens, ordered by best rank in *either* stage | Unpaid-tier requests succeed 3/3 at 5,000 and 0/3 at 6,500; vector-rank order would drop the BM25-only 北海道 chunk the hybrid stage exists to rescue | A wider pool (fails outright); vector-rank ordering (drops the gold chunk); 3,000 (only 8/14 keep their top passage) |
| 2026-09-23 | Voyage tokenizer vendored into `rag/assets/`, loaded with `tokenizers` alone | The budget needs an exact count wherever it runs; transformers is too heavy for a function bundle and `from_pretrained` reaches huggingface.co on a cold start | `transformers`; voyageai's own `count_tokens`; a character-based estimate (wrong by 4.7× here) |
| 2026-09-23 | Table quotes fixed in the prompt; `verify_citations` left untouched | Replaying Q3's retrieval showed a clean quotable row at rank 1 — the gate was right, the model reformatted it. Confirmed on the paid model 2026-09-23: Q3 now passes | A table-aware gate accepting cells from one row (permanently loosens what counts as a quote) |
